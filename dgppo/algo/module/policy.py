...
class PPOPolicy(MultiAgentPolicy):
    ...
    def get_action(self, params: Params, obs: GraphsTuple, rnn_state: Array) -> [Action, Array]:
        '''
        The apply method comes from Flax's Linen module system. In your code, self.dist is a Flax nn.Module (specifically, a TanhNormal, which is a subclass of PolicyDistribution, which is a subclass of nn.Module).

        In Flax, you use .apply(params, ...) to run a module with a set of parameters (often from a trained or initialized model) and inputs. This is standard for Flax models.
        '''
