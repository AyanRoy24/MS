...
'''
Great question! Here’s a concise explanation:

---

### **Why DGPPO and HCBFCRPO Don’t Use `cost-weight` and `cost-schedule`**

- **InforMARL** uses a **cost-weight** to penalize costs directly in the reward:  
  `reward' = reward - cost_weight * cost`
- **InforMARLLagr** (and thus DGPPO/HCBFCRPO) uses a **Lagrangian multiplier** to enforce safety constraints, not a fixed cost weight.  
  - The Lagrangian multiplier is **learned** during training to balance reward and constraint satisfaction.
  - The cost penalty is **adaptive**, not fixed by `cost-weight` or scheduled by `cost-schedule`.

#### **In DGPPO/HCBFCRPO:**
- The cost term is handled by the **Lagrangian** (see `ah_lagr` in informarl_lagr.py), not by a static weight.
- The **CBF loss** (controlled by `cbf-weight`, `cbf-schedule`, etc.) is the main safety mechanism.
- The cost is not directly subtracted from the reward with a fixed weight.

#### **In InforMARL:**
- There is **no Lagrangian**; cost is penalized directly via `cost-weight` and optionally scheduled.

---

### **Summary Table**

| Algorithm         | Cost Handling         | Uses `cost-weight`? | Uses Lagrangian? | Uses CBF? |
|-------------------|----------------------|---------------------|------------------|-----------|
| InforMARL         | reward - cost_weight*cost | Yes                | No               | No        |
| InforMARLLagr     | Lagrangian           | No                  | Yes              | No        |
| DGPPO/HCBFCRPO    | Lagrangian + CBF     | No                  | Yes              | Yes       |

---

**In short:**  
> DGPPO and HCBFCRPO do **not** use `cost-weight` or `cost-schedule` because they enforce safety via a **learned Lagrangian multiplier** (and CBF for DGPPO/HCBFCRPO), not a fixed cost penalty.

Let me know if you want a code pointer or more details!
'''