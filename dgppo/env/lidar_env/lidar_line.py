...
'''
For **offline safe MARL** (Multi-Agent Reinforcement Learning), you should **store both `cost` and `Vh`** during data collection.

### Why store both?

- **`cost`**:  
  - This is the *actual* safety violation signal from the environment (e.g., collisions, constraint violations).
  - It is needed for offline policy evaluation, constraint satisfaction checks, and for training value functions or critics that estimate cost.

- **`Vh` (CBF value)**:  
  - This is the *learned* safety value function (the CBF estimate) produced by your value network.
  - Storing `Vh` allows you to analyze, debug, and potentially use it for offline policy improvement, constraint shaping, or as a feature for other models.

### Typical workflow:
- **Store `cost`**: Always, as it is the ground-truth safety signal.
- **Store `Vh`**: Recommended, especially if you want to analyze how well your learned CBF matches the true cost, or if you want to use it for offline safe policy learning or constraint shaping.

### Summary Table

| What to store | Why?                                      |
|---------------|-------------------------------------------|
| `cost`        | Ground-truth safety signal                |
| `Vh`          | Learned safety value (CBF), for analysis, debugging, or offline learning |

**Best practice:**  
**Store both**. You can always ignore one later, but you cannot recover it if you didn’t store it.

---

**In short:**  
For offline safe MARL, store both `cost` and `Vh` in your dataset.
